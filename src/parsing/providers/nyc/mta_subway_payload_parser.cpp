...
bool parse_mta_subway_payload(const String &message, ProviderPayload &out) {
  ...
  String directionLabel = extract_json_string_field(message, "label");
  if (directionLabel.length() == 0) {
    directionLabel = extract_json_string_field(message, "destination");
  }
  if (directionLabel.length() == 0) {
  ...
  if (label.length() == 0) {
    if (direction == "N") label = "Uptown";
    else if (direction == "S") label = "Downtown";
  }
